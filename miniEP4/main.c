...
int main(int argc, char* argv[])
{
    ...
    uint64_t param_total_time, sum_time, sum_time_sq;
    int i, j, k;
    ...
    for (j = 0; j < num_algorithms; ++j)
    {
        ...
        for (k = 0; k < NUM_EXECUTIONS; ++k)
        {
            ...
            sum_time_sq += (time2-time1)*(time2-time1);
            statistics_print(num_threads, num_accesses);
            printf("---------------------\n");
        }
        sum_time = sum_time/NUM_EXECUTIONS;
        printf("Average execution time: %lu\n", sum_time);
        printf("Average squared execution time: %lu\n", sum_time_sq); 
    }
    return 0;
}
